...
import { blockPortfolio, menuSidebar, pageposting, portfoliopage } from '../../interfaces/home.interface';
...
function JobsPage({ portfolio, servec, cms, page }: PagePortfolioProps): JSX.Element {
	if (portfolio.post_title == "Ошибка") {
		...
		if (max === 1 || max === null) {
			...
			for (let number = 1; number <= maxpage; number++) {
				let url = '/cms_serves/' + slig + '/?page=' + number;
				if (number === active) {
					items.push(
						<span key={number}
...
export async function getServerSideProps(context) {
	...
	let allBlogs = [];
	const { data: portfolio } = await axios.get<blockPortfolio>(process.env.API_PUBLIC_DOMINS + '/cms_serves/' + name,
		{
			params: {
				page: page
			}
